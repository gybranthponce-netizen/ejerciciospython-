import math

# Leer cantidad de números primos a mostrar
cant_a_mostrar = 0

while cant_a_mostrar <= 0:
    cant_a_mostrar = int(input("Ingrese la cantidad de números primos a mostrar: "))

print("1: 2")  # El primer número primo

cant_mostrados = 1
num = 3  # Empezamos desde 3

while cant_mostrados < cant_a_mostrar:
    es_primo = True

    # Probar divisores desde 3 hasta la raíz cuadrada
    for divisor in range(3, int(math.sqrt(num)) + 1, 2):
        if num % divisor == 0:
            es_primo = False
            break

    if es_primo:
        cant_mostrados += 1
        print(cant_mostrados, ":", num)

    num += 2  # Solo probamos impares

    #findelproceso